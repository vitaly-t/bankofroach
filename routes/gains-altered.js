...
router.get('/balances', function(req, res){
	...
	try{
	    ...
	    pg.connect(connectionString, function(err, client, done) {
	        ...
	        var accountTable = '<table class="table table-striped table-bordered"><tr><th>User</th><th>Balance</th><th>GAINS</th></tr>';

	        var query = client.query("SELECT name, balance, gains FROM bank.accounts;");

			query.on('row', function(row) {
				accountTable = accountTable + '<tr><td>' + row.name + '</td><td>' + row.balance +' </td><td>' + row.gains + '</td></tr>';
			 });
	...
	try{
	    ...
	    pg.connect(connectionString, function(err, client, done) {
			...
			query.on('row', function(row) {
				...
				gains = newBalance - row.balance;

				var subquery = client.query("UPDATE bank.accounts SET balance = $1, gains = $3 WHERE id = $2;", [newBalance, row.id, gains], function (err, result) {
				    if (err) {
				      throw (err);
				    }
				});
